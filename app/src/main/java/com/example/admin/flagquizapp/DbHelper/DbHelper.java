package com.example.admin.flagquizapp.DbHelper;
...
import com.example.admin.flagquizapp.Comon.Common;
import com.example.admin.flagquizapp.Model.Question;
import com.example.admin.flagquizapp.Model.Ranking;
...
public class DbHelper extends SQLiteOpenHelper{
    ...
    public List<Question> getQuestionMode(String mode) {
        ...
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM Question ORDER BY Random() LIMIT %d", limit), null);            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
                String Image = c.getString(c.getColumnIndex("Image"));
